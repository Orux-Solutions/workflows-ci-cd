...
El job `pr-report` depende explícitamente de todos esos jobs, actualiza un único comentario idempotente en el PR y sólo termina en verde
cuando cada resultado es `success` o `skipped` por una condición documentada.
Un fallo, cancelación o resultado inesperado hace fallar el reporte y debe bloquear
...
Los callers deben apuntar a `workflows-ci-cd/.github/workflows/ci.yml@main`, declarar
el perfil correcto y configurar el check de calidad como required check
