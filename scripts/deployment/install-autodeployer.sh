...
[[ "$(printf '%s\n%s\n' "${minimum_version}" "${compose_version}" | sort -V | head -n1)" == "${minimum_version}" ]] || {
  echo "Docker Compose ${minimum_version}+ es obligatorio para include (actual: ${compose_version})" >&2
  exit 1
}
